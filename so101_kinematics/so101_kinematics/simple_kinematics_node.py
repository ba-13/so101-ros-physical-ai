...
class SimpleKinematicsNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("traj_cmd_topic", "/follower/trajectory_controller/joint_trajectory")
        ...
        self.joint_sub = self.create_subscription(
            JointState, joints_topic, self.joint_state_cb, 10, callback_group=self.cb_group
        )
        self.ee_target_sub = self.create_subscription(
            PoseStamped, ee_target_topic, self.ee_target_cb, 10, callback_group=self.cb_group
        )
        ...
        # Timer for FK (100 Hz)
        self.timer = self.create_timer(1.0 / 100.0, self.fk_loop, callback_group=self.cb_group)
    ...
    def ee_target_cb(self, msg: PoseStamped):
        if not self.has_joint_feedback:
            self.get_logger().warn("Cannot compute IK, no joint feedback yet.", throttle_duration_sec=2.0)
            return
        ...
        T_goal[:3, :3] = Rotation.from_quat([quat.x, quat.y, quat.z, quat.w]).as_matrix()
        
        q_target = self.solver.solve_goal(self.q_measured, T_goal, n_iters=50)
        self.publish_trajectory(q_target, duration_sec=0.1) # 10Hz target -> 0.1s transition
    ...
    def execute_go_to_joints(self, goal_handle):
        ...
        duration_sec = target_point.time_from_start.sec + target_point.time_from_start.nanosec * 1e-9
        if duration_sec <= 0.0:
            duration_sec = 1.0 # default duration if not specified
            
        # Send to controller
        self.publish_trajectory(target_positions, joint_names=trajectory.joint_names, duration_sec=duration_sec)
